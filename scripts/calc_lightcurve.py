...
a = argparse.ArgumentParser(
    description="Take list of FITS files and derive lightcurve for given sky coordinate."
)
a.add_argument(
    "--ra",
    metavar="ra",
...
for f in files:
    ...
    for hdu in hdulist[1:]:
        ...
        ind_real = np.ix_(
            np.array([0]),
            np.arange(hdu.header["NAXIS4"]),
            np.arange(hdu.header["NAXIS3"]),
            np.array(inds[0]),
            np.array(inds[1]),
        )
        ind_imag = np.ix_(
