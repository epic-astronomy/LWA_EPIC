...
void
MOFFCuHandler::reset_gcf_tex(int p_gcf_dim, float* p_gcf_2D_ptr)
{
    if (is_gcf_tex_set) {
    ...
    cudaMallocArray(&m_gcf_tex_arr, &m_gcf_chan_desc, p_gcf_dim, p_gcf_dim);
    ...
    const size_t spitch = p_gcf_dim * sizeof(float);
    cudaMemcpy2DToArray(m_gcf_tex_arr, 0, 0, p_gcf_2D_ptr, spitch, p_gcf_dim * sizeof(float), p_gcf_dim, cudaMemcpyHostToDevice);
...
void
MOFFCuHandler::reset_data(int p_nchan, size_t p_nseq_per_gulp, int p_gcf_dim, float* p_antpos_ptr, float* p_phases_ptr, float* p_gcf_2D_ptr)
{
    ...
    if (p_gcf_2D_ptr != nullptr) {
        std::cout << "GPU resetting texture\n";
        reset_gcf_tex(p_gcf_dim, p_gcf_2D_ptr);
    }
    cuda_check_err(cudaPeekAtLastError());
...
void MOFFCuHandler::allocate_out_img(size_t p_nbytes){
    if(is_out_mem_set){
...
void
MOFFCuHandler::process_gulp(uint8_t* p_data_ptr, size_t p_buf_size, float* p_out_ptr, size_t p_out_size)
{
    ...
    void* args[] = {
        &m_f_eng_cu, &m_antpos_cu, &m_phases_cu, &m_nseq_per_gulp, &m_nchan,
        // &img_data,
        &m_gcf_tex
    };
