 ...
 */
__device__ float
gcf_pixel_integral(cudaTextureObject_t gcf_tex, float dx, float dy, unsigned int d_per_pixel, int nsteps = 5)
{
    float sum = 0;
 ...
 */
__global__ void
compute_gcf_elements(float* out, float* antpos, int chan0, float lmbda_scale, cudaTextureObject_t gcf_tex, int grid_size /*unused*/, int support = 3, int nants = LWA_SV_NSTANDS)
{
    ...
    for (int ant = grp_rank; ant < nants; ant += grp_size) {
        ...
        out[channel_idx * nants * nelements + ant * nelements + grp_thread_rank] = integral;
 ...
 */
__global__ void
compute_avg_gridding_kernel(float* grid_elems, float* out_kernel, int nchan, int support_size)
{
...
inline __host__ __device__ float4
make_float4(__nv_bfloat162 a, __nv_bfloat162 b)
{
...
/// @return T
template<typename T, class FFT, std::enable_if_t<std::is_same<__half2, typename FFT::output_type::value_type>::value, bool> = true>
