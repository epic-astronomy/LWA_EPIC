  ...
  cudaTextureObject_t gcf_tex)
{
    ...
    for (int ant = tile.meta_group_rank(); ant < NStands;
        ...
        if (!((0 <= int(antx + u) <= size_of<FFT>::value) &&
            ...
            scale = tex2D<float>(
              gcf_tex, abs((int(antx + u) + 0.5 - antx) * inv_half_support), abs((int(anty + v) + 0.5 - anty) * inv_half_support));
  ...
  cudaTextureObject_t gcf_tex)
{
    ...
    for (int ant = 0; ant < NStands; ++ant) {
        ...
        auto phase_ant = phases[ant];
        float scale =
          tex2D<float>(gcf_tex, abs(ant_distx * inv_half_support), abs((anty - (threadIdx.y + 0.5)) * inv_half_support));

        typename FFT::value_type temp;

        __cms_f<half2>(temp.x, float2{ phase_ant.x, phase_ant.y }, f_eng[ant].X, scale);
        __cms_f<half2>(temp.y, float2{ phase_ant.z, phase_ant.w }, f_eng[ant].Y, scale);
    ...
    for (int ant = tile.meta_group_rank(); ant < NStands;
        ...
        if (is_cell_valid) {
            float scale = tex2D<float>(
              gcf_tex, abs((int(antx + u) + 0.5 - antx) * pix2m), abs((int(anty + v) + 0.5 - anty) * pix2m));
            ...
            auto phase_ant = phases[ant];
            __cms_f(temp_data.x, float2{ phase_ant.x, phase_ant.y }, f_eng[ant].X, scale);
            __cms_f(temp_data.y, float2{ phase_ant.z, phase_ant.w }, f_eng[ant].Y, scale);
            ...
            if ((int(antx + u) + (int(anty + v) - offset) * size_of<FFT>::value) >
                ...
                printf("Invalid antenna: %f %f %f %f %f %f %d\n", antx, anty, u, v, antx + u, anty + v, size_of<FFT>::value);
            }
            atomicAdd(
  ...
  ImageDiv Div = UPPER)
{
    ...
    for (int ant = this_thread_ant; ant < NStands; ant += nants_per_pass) {
        ...
        __cms_f(temp_data.x, float2{ phase_ant.x * c - phase_ant.y * s, phase_ant.y * c + phase_ant.x * s }, f_eng[ant].X, scale);
        __cms_f(temp_data.y, float2{ phase_ant.z * c - phase_ant.w * s, phase_ant.w * c + phase_ant.z * s }, f_eng[ant].Y, scale);
