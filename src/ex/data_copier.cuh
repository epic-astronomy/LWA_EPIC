#ifndef DATA_COPIER_CUH
#define DATA_COPIER_CUH
#include "constants.h"
#include "types.hpp"
 ...
 * @tparam C A common type to cast source and destinations if T and P are different
 * @param tb Thread block
 * @param src Pointer to source memory
 * @param dst Pointer to destination memory
 * @param nelements Total elements of the type T to be copied if T and P are equal otherwise
 * number of elements of type C. This will enable. vectorized copies
 * @return void
 */
template<typename C = void, unsigned int ThreadTileSize, typename T, typename P, std::enable_if_t<(ThreadTileSize & (ThreadTileSize - 1)) == 0, bool> = true>
__device__ inline void
memcpy_dx(cg::thread_block tb, P* dst, const T* src, size_t nelements, unsigned int tile_rank = 0)
{
    auto tile = cg::experimental::tiled_partition<ThreadTileSize>(tb);
    if (tile.meta_group_rank() != tile_rank)
        return;
    static_assert(!(std::is_same<T, P>::value == false && std::is_void<C>::value == true),
                  "For different source and destination data types, a common type for the copy is required.");
 ...
 *
 * All the data will be arranged in a sequence: f-engine, ant pos, phases. The destination memory can be of anytype. The data sequences will be appropriately casted to their output types.
 *
...
__device__ void
copy_lwasv_imaging_data(cg::thread_block tb, const uint8_t* f_eng_in, const float* antpos_in, const float* phases_in, T* shared_mem, size_t gulp, size_t chan_idx, size_t ngulps_per_seq, size_t nchan, uint8_t*& f_eng_out, float*& ant_pos_out, float*& phases_out, T*& f_eng_x_phases_out)
{
